...
> [!NOTE]
> Для развёртываний с доступом из интернета используйте [обратный прокси](#использование-обратного-прокси) для добавления HTTPS. Также замените `-p 5001:5001` на `-p 127.0.0.1:5001:5001` в команде `docker run` выше, чтобы предотвратить прямой доступ к незашифрованному порту.
...
> [!NOTE]
> Для развёртываний с доступом из интернета используйте [обратный прокси](#использование-обратного-прокси) для добавления HTTPS. Также измените `"5001:5001/tcp"` на `"127.0.0.1:5001:5001/tcp"` в `docker-compose.yml`, чтобы предотвратить прямой доступ к незашифрованному порту.
